...
public class LLIndexList<E> implements IndexList<E>{
	...
	/**
	 * Private method to access the data node at the position given in the internal
	 * linked list. If the list is not empty, he first data node has position 0, the
	 * following data node (if any) has position 1, and so on.
	 * 
	 * @param posIndex
	 *            the index of the position being accessed.
	 * @return reference to the data node in the given position of the internal
	 *         linked list.
	 **/
	 ...
	 * 
	 * @param i
	 *            the index of the position where the new element is to be inserted.
	 * @param e
	 *            the new element to insert.
	 * @throws IndexOutOfBoundsException
	 *             if the index i does not corresponds to the index of a valid
	 *             position to insert…
	 **/
	public void add(int index, E e) throws IndexOutOfBoundsException {
		if (index < 0 || index > internalLL.length())
			throw new IndexOutOfBoundsException(
					"add: " + "index=" + index + " is out of bounds. size = " + internalLL.length());
		...
		if (index == 0)
			...
			Node<E> nodoPrevio = getDataNodeAtPosition(index - 1);
			internalLL.addNodeAfter(nodoPrevio, nuevoNodo);
		}
	}
	...
	public void add(E e) {
		Node<E> nuevoNodo = internalLL.createNewNode();
		nuevoNodo.setElement(e);
		Node<E> nodoPrevio = internalLL.getLastNode();
		internalLL.addNodeAfter(nodoPrevio, nuevoNodo);
	...
	//	int i = 0;
		for (int i = 0; i < size(); i++) {
			result[i] = this.getDataNodeAtPosition(i);
